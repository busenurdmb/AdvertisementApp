...
namespace AdvertisementApp.UI.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> SignUp()
        {
            ...
            {
             Genders = new SelectList(response.Data,"Id","Definiiton")
            };
            return View(model);
        }
        [HttpPost]
        public async Task<IActionResult> SignUp(UserCreateModel models)
        {
           var result= _userCreateModelValidator.Validate(models);
            if (result.IsValid)
            {
                var dto = _mapper.Map<AppUserCreateDtos>(models);
            ...
            var response = await _genderService.GetAllAsync();
            models.Genders = new SelectList(response.Data, "Id", "Definiiton",models.GenderId);
        ...
        public async Task<IActionResult> SignIn(AppUserLoginDto dto)
        {
           var result =await _appUserService.CheckUserAsync(dto);
            ...
            {
            var roleresult=   await _appUserService.GetRolesByIdAsync(result.Data.Id);
                //ilgili kullanıcın rollerini çekicez
